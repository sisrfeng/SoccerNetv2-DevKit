    ...
    def extractFeatures(self                 ,
        ...
        if "TF2" in self.back_end:
            ...
            if self.grabber == "skvideo":
                videoLoader = Frame(path_video_input,
                                    FPS       = self.FPS       ,
                ...
                videoLoader = FrameCV(path_video_input,
                                      FPS       = self.FPS       ,
                                      transform = self.transform ,
                                      start     = start          ,
                                      duration  = duration       ,
            ...
            # predict the features from the frames (用户要adjust batch size for smaller GPU)
...
if __name__ == "__main__":
    ...
    featureER = VideoFeatureExtractor(feature   = args.features  ,
    ...
    featureER.extractFeatures(path_video_input     = args.path_video    ,
    ...
    if args.PCA is not None or args.PCA_scaler is not None:
        PCAer = PCAReducer(pca_file = args.PCA,
                           scaler_file = args.PCA_scaler,
                          )

        PCAer.reduceFeatures(input_features  = args.path_features ,
                             output_features = args.path_features ,
                             overwrite       = args.overwrite     ,
                            )

# import numpy as np
# np.load("莫某/Football/SoccerNet/england_epl/2014-2015/2015-02-21 - 18-00 Chelsea 1 - 1 Burnley/1_ResNET_TF2.npy")
